...
def test_process_player_futures_keyerror(bad_environ):
    event = sns_nfl_event()
...
def test_process_player_futures_eventerror(environ):
    response = src.handler.process_player_futures.process_player_futures(None, None)
    assert response["statusCode"] == 500
